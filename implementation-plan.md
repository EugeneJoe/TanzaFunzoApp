...
admin creates a class in a module → composes the page from blocks → previews it exactly as a fellow → publishes → releases to cohort → fellow self-signs up (auto-assigned to cohort) → opens class, completes assessment (MC auto-scored, short answer queued) → AI drafts the short-answer grade → admin edits/approves/releases → fellow sees feedback → both dashboards update → admin re-weights an aptitude → scores recompute.
...
9. English only, no i18n scaffolding. Desktop-first UI; responsive fallback is enough.
...
- [ ] `next build` first-load JS for `/learn/class/[classId]` ≤ 200 KB gzipped (NFR-1 budget; check build output).
...
Tasks: full seed — Government Engagement module with 3 classes (class 1 fully authored to mirror §7.1/§7.3 content, incl. the 4-question assessment with rubric; classes 2–3 lighter, class 3 locked with future `release_at`); 4 fellows (Amina Juma, Neema Mushi, Joseph Mrema, Grace Kimaro) with submissions, released grades, and score histories shaped so the admin dashboard shows ≥1 "needs support" and 1 "exceptional" flag; run the full demo-path checklist (§9) against **production**; fix only what the checklist catches.
...
3. **PWA shell:** installable, app-shell cached, offline shows a friendly fallback (not white screen). AC: Lighthouse PWA installable check passes; airplane-mode reload of a visited page shows shell + notice.
4. **Case studies:** case-study block, submission (file/text), grading reuses Stage 4 queue. AC: graded case study produces signals like assessments.
5. **Question bank:** list/reuse questions across assessments. 6. **Cloudflare Stream** provider. (5–6 only if everything above is done.)
...
6. Start assessment → answer 3 MC + 1 short answer → submit → MC portion scored instantly on the results view.
7. As admin: `/admin/grading` → open the new submission → Draft with AI → edit feedback → Approve and release.
8. As the fellow: `/me` shows score + feedback; dashboard shows aptitude scores with signal counts.
9. As admin: open the (duplicated) assessment → change Q1 weights (e.g. Technical 100 → 60/Strategic 40) → save → fellow's Technical/Strategic scores shift on both dashboards; `audit_log` has the change.
10. Reload everything on a throttled "Fast 3G" profile — class page readable in ≤ 3 s.
