...
**Update 10 Jul:** the design handoff was revised — the fellow "Your development" content becomes a dedicated **Dashboard** page (`2C`), the submissions list splits out to a **My coursework** screen (`2E`), and the admin Cohorts screen (`3B`) gains an **add-cohort form**. Fellows now land on the dashboard after login. Executed as **Stage 9** (implementation-plan.md §11), after Stage 8. Password reset remains cut (reaffirmed — no self-service flow for the beta).
