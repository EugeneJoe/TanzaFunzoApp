...
import { getCompletedClassIds } from "@/lib/completion";
...
export default async function DashboardPage() {
  ...
  // Only need the count here (module progress caption), not full result rows.
  const pendingCount = (
    await Promise.all(
      mySubmissions.map(async (sub) => {
        const result = await getSubmissionResult(sub.id);
        return result && !result.allReleased;
      })
    )
  ).filter(Boolean).length;

  const completedIds = await getCompletedClassIds(session.userId, journey);
  const totalCount = journey.length;
  const progressPct = totalCount > 0 ? Math.round((completedIds.size / totalCount) * 100) : 0;
      ...
      <div className="flex flex-col gap-6">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <p className="flex items-center gap-1.5 text-sm text-muted-foreground">
            {completedIds.size} of {totalCount} classes
            {pendingCount > 0 && ` · ${pendingCount} assessment${pendingCount === 1 ? "" : "s"} pending`}
            <InfoTip>
              How much of your cohort&rsquo;s curriculum you&rsquo;ve completed — not a grade. A class
              with an assessment counts once you&rsquo;ve submitted it; a class with no assessment counts
              once you&rsquo;ve opened it. &ldquo;Pending&rdquo; counts submissions still waiting on
              Tanza&rsquo;s grading.
            </InfoTip>
          </p>
        </div>
      </div>
